            ...
            <table class="table mt-4 border-top border-bottom text-muted">
              <tbody>
                <tr>
                  ...
                  <td class="text-end border-0 px-0 pt-4">NT${{ cart?.total.toLocaleString('zh-TW') }}</td>
                </tr>
                <tr>
        ...
        <div class="col-md-6">
          <form v-if="step === 1" @submit.prevent="handleSubmit">
            <div class="mb-2">
              ...
              <input v-model="form.email" required type="email" class="form-control" id="email" placeholder="example@gmail.com" />
              <span v-if="!isEmailValid && isSubmitted" class="text-danger small mt-1"> 請輸入正確的電子信箱 </span>
            </div>
            <div class="mb-2">
              <label for="name" class="text-muted mb-0">姓名</label>
              <input v-model="form.name" required type="text" class="form-control" id="name" placeholder="王漂亮" />
              <span v-if="!isNameValid && isSubmitted" class="text-danger small mt-1"> 請輸入姓名 </span>
            </div>
            <div class="mb-2">
              <label for="tel" class="text-muted mb-0">手機</label>
              <input v-model="form.tel" required type="text" class="form-control" id="tel" placeholder="0912345678" />
              <span v-if="!isPhoneValid && isSubmitted" class="text-danger small mt-1"> 請輸入正確的手機號碼 </span>
            </div>
            <div class="mb-2">
              <label for="address" class="text-muted mb-0">地址</label>
              <input v-model="form.address" required type="text" class="form-control" id="address" placeholder="高雄市新興區" />
              <span v-if="!isAddressValid && isSubmitted" class="text-danger small mt-1"> 請輸入正確的地址 </span>
            </div>
            <div class="mb-2">
          ...
          <div class="d-flex flex-column-reverse flex-md-row mt-4 justify-content-between align-items-md-center align-items-end w-100">
            <template v-if="step === 1">
              <RouterLink to="/cart" class="text-dark mt-md-0 mt-3"><i class="fas fa-chevron-left me-2"></i>返回</RouterLink>
              <button
                @click="handleNextStep"
                :disabled="isSubmitting || !isEmailValid || !isNameValid || !isPhoneValid || !isAddressValid"
                type="button"
            ...
            <template v-else>
              <button @click="step = 1" type="button" class="link-dark bg-transparent border-0 mt-md-0 mt-3 p-0">
                <i class="fas fa-chevron-left me-2"></i>返回
              </button>
              <button @click="handleProcessPayment" :disabled="isProcessingPayment" type="button" class="btn btn-dark py-3 px-7">
                結帳
              </button>
            </template>
          </div>
